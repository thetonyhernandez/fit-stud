          ...
          <div style={{overflowY:"auto",flex:1,padding:"16px 20px 40px"}}>
            {mealPlanStep==="questions"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <button onClick={async()=>{
                ...
                try{
                  const prompt="Create a 7-day meal plan. Profile: Height: "+(mealProfile.height||"not specified")+", Weight: "+(mealProfile.weight||"not specified")+", Goal: "+mealProfile.goal+", Diet: "+mealProfile.dietType+", Activity: "+mealProfile.activityLevel+", Meals per day: "+mealProfile.mealsPerDay+(mealProfile.allergies?", Avoid: "+mealProfile.allergies:"")+". Return ONLY this JSON structure, nothing else: {week1:{monday:{meals:[{time:'7:00 AM',name:'Breakfast',foods:[{item:'Oats',amount:'1 cup',calories:300,protein:10,carbs:54,fat:5}],totalCalories:500,totalProtein:35,totalCarbs:60,totalFat:10}]},tuesday:{meals:[]},wednesday:{meals:[]},thursday:{meals:[]},friday:{meals:[]},saturday:{meals:[]},sunday:{meals:[]}},week2:{monday:{meals:[]},tuesday:{meals:[]},wednesday:{meals:[]},thursday:{meals:[]},friday:{meals:[]},saturday:{meals:[]},sunday:{meals:[]}},week3:{monday:{meals:[]},tuesday:{meals:[]},wednesday:{meals:[]},thursday:{meals:[]},friday:{meals:[]},saturday:{meals:[]},sunday:{meals:[]}},week4:{monday:{meals:[]},tuesday:{meals:[]},wednesday:{meals:[]},thursday:{meals:[]},friday:{meals:[]},saturday:{meals:[]},sunday:{meals:[]}},dailyTotals:{calories:2000,protein:150,carbs:200,fat:65},notes:''}. Fill in real meals for all 28 days based on the profile. Each meal needs time, name, foods array, and totals.";
                  const res=await fetch("/api/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5",max_tokens:6000,system:"You are a sports nutritionist. Return ONLY valid JSON, no markdown, no extra text.",messages:[{role:"user",content:prompt}]})});
                  const data=await res.json();const raw=data.content?.find(b=>b.type==="text")?.text||"";
